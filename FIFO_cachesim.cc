...
static void help()
{
  ...
  std::cerr << "where sets, ways, and blocksize are tailitive integers, with" << std::endl;
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)   // check if address is in cache
{
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  size_t tag = (addr >> idx_shift) | VALID;
  uint64_t victim = -1;
  if(sizes[idx] != ways) {      // check if full
    victim = tags[idx*ways + sizes[idx]];     
    tags[idx*ways + sizes[idx]] = tag;
    sizes[idx]++;
  } else {        // full, find the next index which is going to be updated
    victim = tags[idx*ways + tails[idx]];
    tags[idx*ways + tails[idx]] = tag;
    tails[idx] = (tails[idx] + 1) % ways; // find the next index which is going to be updated
  }
  return victim;
}
...
uint64_t fa_cache_sim_t::victimize(uint64_t addr)
{
  ...
  int victim = -1;
  if(size != ways) {
    victim = tags[idx*ways + size];
    tags[idx*ways + size] = tag;
    sizes++;
  } else {
    victim = tags[idx*ways + tail];
    tags[idx*ways + tail] = tag;
    tail = (tail + 1) % ways;
  }
  return victim;
}
/*
each cache line have a counter, every access will renew the counter, if we want to find fifo, we need to find the next counter which is going to be replaced.
*/
