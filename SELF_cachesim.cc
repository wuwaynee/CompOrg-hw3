...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)) {
      memory_maps[idx][i] = ++maxs[idx];    // added
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  if(sizes[idx] < ways) {// if sizes[idx] isn't full
      ...
      int max_i = 0;    // most recently used
      unsigned long long max = memory_maps[idx][0];   // *
      for(unsigned i = 0; i < ways; i++) {
        if(memory_maps[idx][i] > max) {   // < min to > max, get biggest count
          max_i = i;                      // * find max count
          max = memory_maps[idx][i];      // *
        }      
      }
      memory_maps[idx][sizes[idx]] = ++maxs[idx];
      victim = tags[idx*ways + max_i];    // *
      tags[idx*ways + max_i] = tag;       // *
    }
    return victim;    
}
...
/*
each cache line have a counter, every access will renew the counter, if we want to find mru, we need to update the biggest counter.
*/