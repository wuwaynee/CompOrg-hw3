...
cache_sim_t* cache_sim_t::construct(const char* config, const char* name)   // initialize
{
...
void cache_sim_t::init()
{
  ...
  // added
  memory_maps = new unsigned long long*[sets];
  for(unsigned i = 0; i < sets; i++)
    memory_maps[i] = new unsigned long long[ways];
  maxs = new unsigned long long[sets]();
  sizes = new unsigned int[sets]();
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)   // find if the address is in the memory cache
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)) {
      memory_maps[idx][i] = ++maxs[idx];    // added, maxs count
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  if(sizes[idx] < ways) {         // if sizes[idx] isn't full
      memory_maps[idx][sizes[idx]] = ++maxs[idx];   // maxs is the counter, memory_maps is memory counter
      victim = tags[idx*ways + sizes[idx]];         // the tag which is going to be updated
      tags[idx*ways + sizes[idx]] = tag;            // update the tag
      sizes[idx]++;                                 // next index
  }
    else{               // sizes[idx] is full, find lru 
      int min_i = 0;    // least recently used
      unsigned long long min = memory_maps[idx][0];
      for(unsigned i = 0; i < ways; i++) {      // get smallest counter
        if(memory_maps[idx][i] < min) {
          min_i = i;        // find min count
          min = memory_maps[idx][i];
        }      
      }
      memory_maps[idx][sizes[idx]] = ++maxs[idx];
...
void cache_sim_t::access(uint64_t addr, size_t bytes, bool store)     // memory access
{
...
/*
each cache line have a counter, every access will renew the counter, if we want to find lru, we need to update the least counter.
*/